      ...
      <section className="h-screen w-full relative flex flex-col overflow-hidden border-b border-white/5">
        <header className="relative z-50 bg-[#05070a] border-b border-white/5">
          <div className="container mx-auto px-6 h-20 flex items-center justify-between gap-8">
            ...
            {/* Logo */}
            <div className="flex items-center gap-3 group cursor-pointer shrink-0 text-white uppercase tracking-tighter text-center">
              ...
              <div className="flex flex-col items-center leading-none">
                ...
                <span className="text-[9px] text-white font-bold tracking-[0.3em] mt-1 opacity-80 uppercase">Security Solutions</span>
              </div>
            </div>
            
            {/* Catalog Container (Added pt-4 to bridge the gap) */}
            <div className="relative group" onMouseLeave={() => setActiveCat(null)}>
              ...
              {/* Bridge - Невидимый слой, закрывающий дыру в 15px между кнопкой и меню */}
              <div className="absolute top-full left-0 w-full h-[16px] invisible group-hover:visible" />
                ...
                {activeCat && (
                  <div className="p-10 min-w-[450px] bg-[#0a0c10] border-l border-white/5 animate-in fade-in duration-200">
                    ...
                    <div className="grid grid-cols-2 gap-x-12 gap-y-4 text-slate-400 font-bold text-sm">
                      {activeCat.sub.map((item: string) => (<a key={item} href="#" className="hover:text-blue-500">{item}</a>))}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-6 text-white font-black">
              ...
              <button className="bg-blue-600 p-3 rounded-xl relative hover:bg-blue-700 transition-all shadow-[0_5px_15px_rgba(37,99,235,0.2)]"><ShoppingCart size={20} /><span className="absolute -top-1 -right-1 bg-white text-blue-600 text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">0</span></button>
            </div>
          </div>
        </header>
        ...
        <section className="flex-1 flex flex-col md:flex-row items-center container mx-auto px-6 overflow-hidden">
          <div className="w-full md:w-1/2 flex flex-col justify-center py-12 z-20">
            <div className="max-w-4xl">
              ...
              <button className="w-fit bg-blue-600 text-white px-10 py-4 rounded-2xl font-black text-lg hover:bg-blue-700 transition-all flex items-center gap-3 active:scale-95 uppercase">
          ...
          <div className="w-full md:w-1/2 h-full flex items-center justify-center relative p-8">
            <div className="absolute w-[80%] h-[80%] bg-blue-600/10 blur-[120px] rounded-full"></div>
